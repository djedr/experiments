...
const self = callable({args: 'fileName', body: c=> [
    ...
    (err, source) => {
        ...
        let defined = applyRegex(source, macroDefRegExp, (source, index, macroName) => {
            ...
            generator += `function ${macroName}(${args.match}) {${body.match}}\n`;

            macros[macroName] = eval(`(function (${args.match}) {${body.match}})`);
            return { index, value: '' };
        });

        let substituted = applyRegex(defined, macroInvRegExp, (source, index, macroName, partial) => {
            const args = matchParens(source, index, '(', ')');
            ...
            generator += `output += \`${partial}\${${macroName}(${args.match})}\`;\n\n`;
        ...
        eval(`(() => { let output = ''; ${generator}; console.log(output); })()`);
   }
]});
